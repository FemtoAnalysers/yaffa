...
void SimLambda1520(
    ...
    //    momentum smearing
    std::string outFilePath = "SimLambda1520_KinemXi1530_new") {
    ...
    // Lambda 1520
    Pythia8::Particle lambda1520Feature;
    ...
    for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
            ...
            if (pythia.event[iPart].id() == 3122) {
                ...
                for (int jPart = 3; jPart < pythia.event.size(); jPart++) {
                    if (abs(pythia.event[jPart].id()) == 211) {
                        Pythia8::Particle pion = pythia.event[jPart];
                        TLorentzVector momPion = TLorentzVector(pion.px(), pion.py(), pion.pz(), pion.e());

                        // acceptance and momentum cuts on the decay products
                        if (lambda.pT() >= 0.3 && abs(lambda.eta()) <= 0.8 && pion.pT() >= 0.3 &&
                            abs(pion.eta()) <= 0.8) {
                            TString histoKey =
                                std::to_string(pythia.event[iPart].id()) + std::to_string(pythia.event[jPart].id());
        ...
        // AntiLambda 1520
        Pythia8::Particle antiLambda1520;
    ...
    // TH2F *smearMomentumMatrix02 =
    // static_cast<TH2F*>(pairList02->FindObject("MomentumResolutionSE_Particle0_Particle2")); TString smearMatrix02 =
    // "2113122_smear_matr"; smearMatrices.push_back(smearMomentumMatrix02);

    // TList *pairList12 = dynamic_cast<TList*>(QAList->FindObject("QA_Particle1_Particle2"));
    // TH2F *smearMomentumMatrix12 =
    // static_cast<TH2F*>(pairList12->FindObject("MomentumResolutionSE_Particle1_Particle2")); TString smearMatrix12 =
    // "-2113122_smear_matr"; smearMatrices.push_back(smearMomentumMatrix12);

    // TList *pairList03 = dynamic_cast<TList*>(QAList->FindObject("QA_Particle0_Particle3"));
    // TH2F *smearMomentumMatrix03 =
    // static_cast<TH2F*>(pairList03->FindObject("MomentumResolutionSE_Particle0_Particle3")); TString smearMatrix03 =
    // "211-3122_smear_matr"; smearMatrices.push_back(smearMomentumMatrix03);

    // TList *pairList13 = dynamic_cast<TList*>(QAList->FindObject("QA_Particle1_Particle3"));
    // TH2F *smearMomentumMatrix13 =
    // static_cast<TH2F*>(pairList13->FindObject("MomentumResolutionSE_Particle1_Particle3")); TString smearMatrix13 =
    // "-211-3122_smear_matr"; smearMatrices.push_back(smearMomentumMatrix13);
    ...
    //   }
    //   for(int iBin=0; iBin<hSEPairsSmeared[keys[iKey]]->GetNbinsX(); iBin++) {
    //     hSEPairsSmeared[keys[iKey]]->SetBinError(iBin+1,
    //     TMath::Sqrt(hSEPairsSmeared[keys[iKey]]->GetBinContent(iBin+1)));
    //   }
    // }
