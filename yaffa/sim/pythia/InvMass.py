...
double fit_FemtoSillWeightedLK(double *x, double *par)
{
  ...
  if (t >= enThr2)
  {
    ...
    denominator2 = pow((t * t - par[2] * par[2]), 2) + pow(numerator + par[0] * TMath::Sqrt(t * t - enThr1 * enThr1), 2);
...
//------------------------------------------
void InvMass(TString homeFolder = "/Users/sartozza", int NumSystIter = 1, int REBINMC = 1)
{
  SetStyle(false, false);

  TFile *OutputFile = new TFile("fOutput_InvMass.root", "recreate");
  ...
  int UsePbPb = 0;
  InputTree = TFile::Open(fName_Output + TString::Format("OutputTree_rebinMC2rebinDATA1_Fit%i_Back0_Omega%i_EffRangeZero%i_UsePbPb%i_Resonance.root", TypeOfFit, int(WithOmega), int(IsEffRangeZero), UsePbPb));
  ...
  double MassOmegaEnergy_mean = sqrt(MassOmega_mean * MassOmega_mean + mL * mL) + sqrt(MassOmega_mean * MassOmega_mean + mK * mK);

  double MassOmega_sig_stat = hMassOmegafFullFemto_stat->GetStdDev();
  double MassOmega_sig_tot = hMassOmegafFullFemto->GetStdDev();
  double MassOmega_sig_syst = TMath::Sqrt(MassOmega_sig_tot * MassOmega_sig_tot - MassOmega_sig_stat * MassOmega_sig_stat);
  ...
  double GammaOmega_sig_syst = TMath::Sqrt(GammaOmega_sig_tot * GammaOmega_sig_tot - GammaOmega_sig_stat * GammaOmega_sig_stat);
  ...
  double HeightOmega_sig_syst = TMath::Sqrt(HeightOmega_sig_tot * HeightOmega_sig_tot - HeightOmega_sig_stat * HeightOmega_sig_stat);

  /// Ξ(1690)
  double MassXi1_mean = hMassXi1fFullFemto_stat->GetMean();
  double MassXi1Energy_mean = sqrt(MassXi1_mean * MassXi1_mean + mL * mL) + sqrt(MassXi1_mean * MassXi1_mean + mK * mK);
  ...
  double HeightXi1_sig_syst = TMath::Sqrt(HeightXi1_sig_tot * HeightXi1_sig_tot - HeightXi1_sig_stat * HeightXi1_sig_stat);

  /// Ξ(1820)
  double MassXi2_mean = hMassXi2fFullFemto_stat->GetMean();
  double MassXi2Energy_mean = sqrt(MassXi2_mean * MassXi2_mean + mL * mL) + sqrt(MassXi2_mean * MassXi2_mean + mK * mK);
  ...
  double HeightXi2_sig_syst = TMath::Sqrt(HeightXi2_sig_tot * HeightXi2_sig_tot - HeightXi2_sig_stat * HeightXi2_sig_stat);
  ...
  hMEDataNorm->Scale(hSEDataNorm->Integral(hSEDataNorm->FindBin(norm1), hSEDataNorm->FindBin(norm2)) / hMEDataNorm->Integral(hMEDataNorm->FindBin(norm1), hMEDataNorm->FindBin(norm2)));
  ...
  hSEMCNorm->Scale(hSEDataNorm->Integral(hSEDataNorm->FindBin(norm1), hSEDataNorm->FindBin(norm2)) / hSEMCNorm->Integral(hSEMCNorm->FindBin(norm1), hSEMCNorm->FindBin(norm2)));
  ...
  hMEMCNorm->Scale(hSEDataNorm->Integral(hSEDataNorm->FindBin(norm1), hSEDataNorm->FindBin(norm2)) / hMEMCNorm->Integral(hMEMCNorm->FindBin(norm1), hMEMCNorm->FindBin(norm2)));
  ...
  for (unsigned uBin = 1; uBin < h_IMMinusMC->GetNbinsX() + 1; uBin++)
  {
    ...
    BinBoundaries[uBin] = sqrt(h_IMMinusMC->GetXaxis()->GetBinUpEdge(uBin) * h_IMMinusMC->GetXaxis()->GetBinUpEdge(uBin) + mL * mL) + sqrt(h_IMMinusMC->GetXaxis()->GetBinUpEdge(uBin) * h_IMMinusMC->GetXaxis()->GetBinUpEdge(uBin) + mK * mK);
  }

  TH1F *h_IMMinusMCEn = new TH1F(TString::Format("h_IMMinusMCEn_var%i", uSyst), TString::Format("h_IMMinusMCEn_var%i", uSyst),
  ...
  // Omega
  double MassOmega_meanEn = sqrt(MassOmega_mean * MassOmega_mean + mL * mL) + sqrt(MassOmega_mean * MassOmega_mean + mK * mK);
  ...
  // Xi 1620
  double kstar_Xi1620 = sqrt(pow(MassXi1620_mean, 4) - 2 * MassXi1620_mean * MassXi1620_mean * mL * mL + pow(mL, 4) - 2 * MassXi1620_mean * MassXi1620_mean * mK * mK - 2 * mL * mL * mK * mK + pow(mK, 4)) / (2 * MassXi1620_mean);
  ...
  f1 = new TF1("fit_axis", "sqrt(x*x*x*x-2*x*x*[0]*[0]+pow([0],4)-2*x*x*[1]*[1]-2*[0]*[0]*[1]*[1]+pow([1],4))/(2*x)", EnMin, EnMax);
  f1->SetParameter(0, mL);
  f1->SetParameter(1, mK);
  TGaxis *axis = new TGaxis(gPad->GetUxmin(), gPad->GetUymax(),
                            gPad->GetUxmax(), gPad->GetUymax(), "fit_axis", 507, "-");
