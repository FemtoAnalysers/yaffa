...
bool IsSelected(const Pythia8::Pythia &pythia, const Pythia8::Particle &part, const YAML::Node &cfgSelections) {
    auto statusrange0 = cfgSelections["status"].IsNull() ? std::array<int, 2>({-300, 300}) : cfgSelections["status"].as<std::array<int, 2>>();
    auto ptrange0 = cfgSelections["pt"].IsNull() ? std::array<double, 2>({0, 9.e9}) : cfgSelections["pt"].as<std::array<double, 2>>();
    auto etarange0 = cfgSelections["eta"].IsNull() ? std::array<double, 2>({-9.e9, 9.e9}) : cfgSelections["eta"].as<std::array<double, 2>>();
    auto yrange0 = cfgSelections["y"].IsNull() ? std::array<double, 2>({-9.e9, 9.e9}) : cfgSelections["y"].as<std::array<double, 2>>();
    auto prodvtxrange0 = cfgSelections["prodvtx"].IsNull() ? std::array<double, 2>({-1., 9.e9}) : cfgSelections["prodvtx"].as<std::array<double, 2>>();

    
    if (!(statusrange0[0] <= part.status() && part.status() <= statusrange0[1])) return false;
    if (!(ptrange0[0] < part.pT() && part.pT() < ptrange0[1])) return false;
    if (!(etarange0[0] < part.eta() && part.eta() < etarange0[1])) return false;
    if (!(yrange0[0] < part.y() && part.y() < yrange0[1])) return false;
    double prodvtx = pow(part.xProd() * part.xProd() + 
                        part.yProd() * part.yProd() + 
                        part.zProd() * part.zProd(), 0.5);
    if (!(prodvtxrange0[0] < prodvtx && prodvtx < prodvtxrange0[1])) return false;
...
void MakeDistr(
    ...
    cfgPart1 = cfg["part1"].IsNull() ? cfg["part0"] : cfg["part1"];
    ...
    for (size_t iEvent = 0; iEvent < nEvents; iEvent++) {
        ...
        for (int iPart = 3; iPart < pythia.event.size(); iPart++) {
            ...
            if (absPdg == pdg0 && IsSelected(pythia, pythia.event[iPart], cfgPart0)) {
                part0.push_back(pythia.event[iPart]);
            } else if (absPdg == pdg1 && IsSelected(pythia, pythia.event[iPart], cfgPart1)) {
