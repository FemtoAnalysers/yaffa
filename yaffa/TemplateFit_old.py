...
from utils.StyleFormatter import SetGlobalStyle, SetObjectStyle, GetROOTColor, DivideCanvas
...
hhDcaPtNamesMC = [
    f'HM_CharmFemto_D{buddy}_TrackCutsMC0/HM_CharmFemto_D{buddy}_TrackCutsMC0/DCAPtBinning/DCAPtBinning',
...
for hhDcaPt_data, hhDcaPt_mc in zip(hhDcaPtList_data, hhDcaPtList_mc):
    ...
    for iPtBin, (ptMin, ptMax) in enumerate(zip(ptMins, ptMaxs)):
        ...
        for source, fData, fDataUnc, fMC in zip(
            sourceList, fractionsInData[iPtBin], fractionsInDataUnc[iPtBin], fractionsInMC[iPtBin]):
    ...
    gFraction[0].SetTitle(';#it{p}_{T} (GeV/c);fraction')
    ...
    for iPtBin, (ptMin, ptMax) in enumerate(zip(ptMins, ptMaxs)):
        ...
        for iSource, (source, frac, fracUnc) in enumerate(
            zip(sourceList, fractionsInData[iPtBin], fractionsInDataUnc[iPtBin])):
