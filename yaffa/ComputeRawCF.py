    ...
    for iRegion, region in enumerate(regions):
        ...
        if f'HMResults{runSuffix}' in GetKeyNames(inFile): # Make correlation functions from FemtoDream
            ...
            if 'multrewMCwithdata' in cfg:
                ...
                dirData = f'HMResults{runSuffixData}/HMResults{runSuffixData}/{fdcomb}/SEMultDist_{fdcomb}'
                hSEmultk[comb][region] = Load(inFileData, dirData)
        ...
        nBins = hMEmultk[comb][region].ProjectionY().GetNbinsX() + 2
        xMin = hMEmultk[comb][region].GetYaxis().GetXmin()
        xMax = hMEmultk[comb][region].GetYaxis().GetXmax()
        hWeights = TH1D(f"Weights{region}_{comb}", f"Weights{region}_{comb}", nBins, xMin, xMax)

        nBins = hMEmultk[comb][region].ProjectionX().GetNbinsX()
        xMin = hMEmultk[comb][region].GetXaxis().GetXmin()
        xMax = hMEmultk[comb][region].GetXaxis().GetXmax()
        hMEreweightk = TH1D(f"MErewdistr{region}_{comb}", f"MErewdistr{region}_{comb}", nBins, xMin, xMax)

        startBinMultRew, endBinMultRew = cfg.get('binmultrew', [0, nBins])
        for iBin in range(startBinMultRew, endBinMultRew): # Loop over underflow, all bins, and overflow
            hSEbinmult = hSEmultk[comb][region].ProjectionX(f'{comb}SEdistr', iBin, iBin)
            hMEbinmult = hMEmultk[comb][region].ProjectionX(f'{comb}MEdistr', iBin, iBin)
