...
def make_plot(plot):
    ...
    while plot.get('ratio', {}).get('enable'): # use if-equivallent while scope to be able to control when to exit
        ...
        if sum([isinstance(obj, (TF2, TH2)) for obj in inObjs]) > 2:
            log.error('Cannot draw more than 2 2D objects. Skipping ratio plot')
            break
    ...
    while plot.get('spread', {}).get('enable'):
        ...
        if len(inObjs) <= 2:
            log.error('Computing the spread for only 2 object doesn\'t make much sense. Skipping spread plot')
            break
        ...
        for iBin in range(hSpread.GetXaxis().FindBin(x1 * 1.0001), hSpread.GetXaxis().FindBin(x2 * 0.9999)):
            ...
            if isinstance(inObjs[0], TH1):
                den = inObjs[0].GetBinContent(iBin + 1)
            elif isinstance(inObjs[0], TF1):
                den = inObjs[0].Eval(inObjs[1].GetBinCenter(iBin + 1))
            spread = np.std(yValues) / den
    ...
    while plot.get('relunc', {}).get('enable'):
        ...
        for iObj, inObj in enumerate(inObjs):
            if isinstance(inObj, TH1):
                ...
                log.error('Relative uncertainties for type %s are not implemented. Skipping this object', inObj.ClassName())
                continue
